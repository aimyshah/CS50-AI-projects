...
def probs_has_parents(person, people, one_gene, two_genes, has_trait):
    ...
    p_father_dont_pass_gene_and_mutate = (1 - prob_father_pass_gene) * PROBS["mutation"]
    ...
    p_mother_dont_pass_gene_and_mutate = (1 - prob_mother_pass_gene) * PROBS["mutation"]

    child_genes = check_how_many_copies(person, one_gene, two_genes)

    # if child_genes is 0, we have four scenarios
    if child_genes == 0:
    ...
    # if child_genes is 1, we have eight scenarios
    elif child_genes == 1:
        probability = (p_father_pass_gene_and_dont_mutate * p_mother_dont_pass_gene_and_dont_mutate) + \
                      ...
                      (p_father_pass_gene_and_do_mutate * p_mother_dont_pass_gene_and_mutate) + \
                      (p_father_pass_gene_and_do_mutate * p_mother_pass_gene_and_dont_mutate) + \
                      (p_father_dont_pass_gene_and_mutate * p_mother_dont_pass_gene_and_dont_mutate) + \
                      (p_father_dont_pass_gene_and_mutate * p_mother_pass_gene_and_do_mutate) + \
                      (p_father_dont_pass_gene_and_dont_mutate * p_mother_pass_gene_and_dont_mutate) + \
                      (p_father_dont_pass_gene_and_dont_mutate * p_mother_dont_pass_gene_and_mutate)

    # if child_genes is 2, we have four scenarios
    elif child_genes == 2:
        probability = (p_father_pass_gene_and_dont_mutate * p_mother_pass_gene_and_dont_mutate) + \
                      (p_father_pass_gene_and_dont_mutate * p_mother_dont_pass_gene_and_mutate) + \
                      (p_father_dont_pass_gene_and_mutate * p_mother_dont_pass_gene_and_mutate) + \
                      (p_father_dont_pass_gene_and_mutate * p_mother_pass_gene_and_dont_mutate)
...
def joint_probability(people, one_gene, two_genes, have_trait):
    ...
    for person in people:
        ...
        if people[person]["mother"] is None:
            ...
            probability *= (probs_has_parents(person, people, one_gene, two_genes, has_trait) * PROBS["trait"][copies_gene][has_trait])
    return probability
